...
function getBinaryName() {
  ...
  if (platform === "darwin") {
    return arch === "arm64" ? "engine-darwin-arm64" : "engine-darwin-amd64";
  } else if (platform === "linux") {
    return arch === "arm64" ? "engine-linux-arm64" : "engine-linux-amd64";
  } else if (platform === "win32") {
    return arch === "arm64"
      ? "engine-windows-arm64.exe"
      : "engine-windows-amd64.exe";
  }
...
function renderTable(stats) {
  ...
  console.log("--------------------------");
  console.log(
    `TOTAL SPACE SAVED: ${formatBytes(totalSaved)} (${(totalSaved / 1024).toFixed(2)} KB)\n`
  );
}

program
  .name("img-size-compress")
  .description("A CLI tool to compress image sizes")
  .version("1.0.0")
